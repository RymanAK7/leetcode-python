...
The time complexity of creating the graph
is O(m), where m is the number of edges.

BFS Traversal (bfs method):
In the worst case, each node and
edge are visited once, resulting
in a time complexity of O(n + m).
The loop may iterate through all nodes
and edges in the worst case, but
...
Create Graph (create_graph method):
Since each node may have multiple neighbors,
the space complexity for the graph
representation is typically O(n + m),
...
The visited set in the bfs method
is used to keep track of visited nodes
during the BFS traversal.
In the worst case, all nodes in the graph
may need to be stored in the visited set,
resulting in a space complexity of O(n).

The queue used for BFS traversal stores
nodes to be visited. In the worst case,
all nodes may need to be enqueued,
resulting in a space complexity of O(n).
